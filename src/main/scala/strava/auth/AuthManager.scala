...
class AuthManager[F[_]](
  ...
  def getValidToken(): F[Either[StravaError, StravaToken]] =
    for
      ...
      result <- tokenOpt match
        case None =>
          F.pure(Left(StravaError.AuthenticationError(
        ...
        case Some(token) =>
          ...
          F.pure(Right(token): Either[StravaError, StravaToken])
    yield result
  ...
  def refreshToken(oldToken: StravaToken): F[Either[StravaError, StravaToken]] =
    ...
    given io.circe.Decoder[StravaToken] = StravaToken.decoderFromApi
    ...
    request.send(backend).flatMap: response =>
      response.body match
        ...
        case Left(error) =>
          ...
          F.pure(Left(StravaError.AuthenticationError(errorMsg)): Either[StravaError, StravaToken])
    .handleErrorWith: e =>
      logger.error("Network error during token refresh", e)
      F.pure(Left(StravaError.NetworkError(
