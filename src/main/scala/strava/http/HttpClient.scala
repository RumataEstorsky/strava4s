...
class HttpClient[F[_]](
  ...
  private def makeRequest[T: Decoder, B: Encoder](
    ...
    for {
      // Rate limiting
      _ <- if (config.enableRateLimiting) rateLimiter.checkAndWait() else A.unit
  ...
  private def executeRequest[T: Decoder, B: Encoder](
    ...
    request.send(backend).flatMap { response =>
      ...
      val rateLimitRemaining = response.header("X-RateLimit-Limit").flatMap(_.toIntOption)
      rateLimiter.recordResponse(rateLimitRemaining) >>
      ...
      if (retryCount < config.maxRetries) {
        ...
        A.pure(Left(StravaError.NetworkError(e.getMessage, Some(e))))
      }
    }
  }
    ...
    response.code match {
      ...
      case StatusCode.Unauthorized =>
        ...
        if (retryCount < config.maxRetries) {
          ...
          A.pure(Left(StravaError.AuthenticationError("Unauthorized")))
        }

      case StatusCode.NotFound =>
        A.pure(Left(StravaError.NotFoundError(
          s"Resource not found: $path"
        )))

      case StatusCode.TooManyRequests =>
        val retryAfter = response.header("Retry-After").flatMap(_.toLongOption)
      ...
      case StatusCode.BadRequest =>
        response.body match {
          case Right(json) =>
            A.pure(Left(StravaError.ValidationError(
              s"Bad request: $json"
            )))
          case Left(error) =>
...
private[http] sealed trait RequestMethod
private[http] object RequestMethod {
